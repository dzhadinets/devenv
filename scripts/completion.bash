...
#DBG 
#set -x
#COMP_WORDS=(devenv-cmake dbg)

get_help()
{
    DEVENV_CMD=${COMP_WORDS[0]}
    #    set +x
    HELP_OUT=($(${DEVENV_CMD} --help | gawk 'match($0, /^\s\s(.+)\t-\s.*/, a) {printf "%s@",a[1]}' | tr ' ' '#' | tr '@' ' '))
    for i in ${!HELP_OUT[@]};
    do
...
complete -F _devenv_get_next devenv-cmake
complete -F _devenv_get_next devenv-aosp
complete -F _devenv_get_next devenv-yocto
